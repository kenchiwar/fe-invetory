  ...
  try {
    ...
    switch (method) {            
      case "GET":
        ...
        let myConfig = {cache : false,...config} as myAxiosRequestConfig;
        if(myConfig.cache) {
      ...
      case "DELETE":
        ...
        break
      case "PATCH":
        response = await axiosInstance.patch<ApiResponse<T>,CacheAxiosResponse<ApiResponse<T>>>(url, data, config);
        break
      default:
